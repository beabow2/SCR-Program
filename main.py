...
#Continuing the FearDisplayApp class with additional methods
...
def SCR_resp(self,rise_begin, rise_end, max_rise_time, situation, target, display_window, time_step=TIME_STEP):
    """
    :param rise_begin:
    :param rise_end:
    :param max_rise_time:
    ...
    self.SCR_df = df[(df[0] >= place_SCR) & (df[0] <= place_SCR + display_window)]
    trough_index = find_peaks(-SCR_start[1])[0]
    self.SCR_response = 0
    ...
    # if we find trough in the space
        ...
        for i in range(len(trough_index)):
            SCR_peaks = SCR_df[trough_index[i] + int(rise_begin / time_step):trough_index[i] + int(rise_begin / time_step) + int(
                max_rise_time / time_step) + 1]
            peak_index = find_peaks(SCR_peaks[1])[0]
            SCR_troughrow = SCR_peaks.iloc[0]
...
# Update CS Response
# noinspection PyGlobalUndefined
def update_GUI(CSP_response, CSM_response):
    CSP_text.config(text=f"CS+ Response:{CSP_response}")
    CSM_text.config(text=f"CS- Response:{CSM_response}")
    diff = round(CSP_response - CSM_response, 4)
    Diff_text.config(text=f"Difference:{diff}")
    reward = round(300 - 100 * diff)
    reward_text.config(text=f"Reward:{reward}")
...
def CS_resp_update(CSP_res_temp, CSM_res_temp, CSP_df_temp, CSM_df_temp, CSP_res_figure_temp, CSM_res_figure_temp):
    ...
    # update CS response shown on the GUI
    update_GUI(CSP_response, CSM_response)
    # graph SCR response
...
# CS+
CSP_text = tk.Label(label_frame, text="CS+ Response:", font=("Times New Roman", 10), bg='white')
CSP_text.grid(row=1, column=1, padx=30, pady=10)

# CS-
CSM_text = tk.Label(label_frame, text="CS- Response:", font=("Times New Roman", 10), bg='white')
CSM_text.grid(row=1, column=2, padx=30, pady=10)
